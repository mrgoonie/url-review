...
const defaults: MakeSlugOptions = {
  delimiter: "-",
  lowercase: true,
  replacements: {},
...
export const makeSlug = (input: string, opt: MakeSlugOptions = {}) => {
  ...
  if (isEmpty(str) || containsChinese(str) || containsEmoji(str)) str = "item";
  ...
  for (const k in opt.replacements) {
    if (opt.replacements[k]) {
      str = str.replace(RegExp(k, "g"), opt.replacements[k] as string);
    }
  }
...
export function makeSlugByName(name: string) {
  try {
    ...
    throw new Error(
      `Make Slug By Name failed: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
