...
import type { TextModel, VisionModel } from "@/lib/ai";
...
export async function startReview(
  ...
  options?: {
    ...
    /**
     * Maximum number of images to extract from the website
     * @default 50
     */
    maxExtractedImages?: number;
    /**
     * Maximum number of links to extract from the website
     * @default 50
     */
    maxExtractedLinks?: number;
    /**
     * AI model to use for text analysis
     */
    textModel?: TextModel;
    /**
     * AI model to use for image analysis
     */
  ...
  try {
    // 2. Get images (max 50)
    const imageUrls = await getAllImages(input.url, {
      delayAfterLoad: 3000,
      excludeSelectors: ["svg", 'img[alt=""]', 'img[src^="data:image/svg"]'],
    });
    const limitedImageUrls = imageUrls.slice(0, 50);

    // 3. Get links (max 50)
    const links = await extractLinks(input.url);
    ...
    const base64 = await imageUrlToBase64(limitedImageUrls[0]);
    if (!base64) console.warn(`No base64 found for image ${limitedImageUrls[0]}`);
    const imagesAnalysis = base64
      ? await Promise.all(
          limitedImageUrls.map(async (imageUrl) => {
            try {
              return await analyzeImageBase64(
                ...
                { model: options?.visionModel || "google/gemini-flash-1.5-8b" }
              );
            } catch (error) {
